...
bool Grid<T>::GetValue(const size_t &x_idx, const size_t &y_idx,
  ...
  CHECK(impl_);  // Grid may be moved.
  CHECK_NOTNULL(value);
  if (x_idx >= static_cast<unsigned int>(impl_->grid_size_.width) || y_idx >= static_cast<unsigned int>(impl_->grid_size_.height)) {
    return false;
  }
...
void Grid<T>::Expand(const cv::Size &delta_size) {
  ...
  for (size_t i = 0; i < static_cast<unsigned int>(impl_->grid_size_.width); ++i) {
    for (size_t j = 0; j < static_cast<unsigned int>(impl_->grid_size_.height); ++j) {
